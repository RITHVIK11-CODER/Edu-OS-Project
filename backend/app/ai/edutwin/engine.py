...
class EduTwinEngine:
    def update(self,state:EduTwinState,evidence:MasteryEvidence)->EduTwinState:
        ...
        if not evidence.correct and evidence.diagnosis_confidence>=.85:mastery-=.05
        mastery=min(1.0,max(0.0,mastery))
        confidence=min(1.0,max(0.0,(.70*state.confidence)+(.30*evidence.diagnosis_confidence)))
        if mastery>=.85 and evidence.correct:learning_state=LearningState.MASTERED
        elif evidence.correct and mastery>=.65:learning_state=LearningState.READY_TO_REASSESS
        elif mastery<.40:learning_state=LearningState.NEEDS_REVIEW
        elif not evidence.correct:learning_state=LearningState.NEEDS_PRACTICE
        else:learning_state=LearningState.DEVELOPING
