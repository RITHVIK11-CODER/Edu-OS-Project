...
class MindTraceEngine:
    ...
    def _classify_incorrect(self,data:MindTraceInput):
        ...
        if self._looks_like_coefficient_error(data):return "coefficient_misinterpretation",["The leading coefficient changes the expected solution structure, but the response treats it as a monic quadratic."],.86,LearningAction.WORKED_EXAMPLE
    ...
    def _looks_like_sign_error(data:MindTraceInput)->bool:
        ...
        expected_negative="-" in expected; answer_negative="-" in answer
        return (expected_negative != answer_negative) or ("+" in expected and "-" in answer) or ("-" in expected and "+" in answer)
    @staticmethod
    def _looks_like_coefficient_error(data:MindTraceInput)->bool:
        return bool(re.search(r"\b2x\^2\b",data.question.replace(" ","")) and "/" in data.correct_answer and "/" not in data.student_answer)
    @staticmethod
    ...
    def _diagnosis_description(code:str,data:MindTraceInput)->str:
        return {"incorrect_factor_pair":f"The response suggests an incorrect factor selection for {data.concept}.","sign_error":"The response suggests that a sign was handled incorrectly.","incomplete_factorization":"The response suggests that the factorization procedure was started but not completed.","arithmetic_error":"The response suggests a numerical calculation error rather than a clearly different concept.","formula_misapplication":"The response suggests that a formula was applied incorrectly to the problem.","coefficient_misinterpretation":"The response suggests the leading coefficient was not incorporated correctly.","conceptual_gap":"The available evidence suggests that the underlying concept needs review."}.get(code,"The available evidence does not support a narrower diagnosis.")
    @staticmethod
