...
async def get_current_user(credentials:HTTPAuthorizationCredentials|None=Depends(bearer_scheme))->dict[str,Any]:
 ...
 try:
  token=credentials.credentials; client=get_authenticated_client(token); auth_user=getattr(client.auth.get_user(token),"user",None)
...
require_student=require_role("STUDENT"); require_teacher=require_role("TEACHER"); require_parent=require_role("PARENT"); require_principal=require_role("PRINCIPAL"); require_admin=require_role("ADMIN")
