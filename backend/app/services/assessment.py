...
def result(attempt_id:str,user_id:str)->dict:
    ...
    return {"attempt_id":attempt_id,"status":attempt["status"],"score":attempt.get("score"),"correct_count":sum(1 for r in rows if r.get("is_correct") is True),"total_questions":len(rows)}
